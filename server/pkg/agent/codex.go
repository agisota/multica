...
func (b *codexBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	go func() {
		...
		threadResult, err := c.request(runCtx, "thread/start", map[string]any{
			...
			"profile":       nil,
			"cwd":           opts.Cwd,
			// Managed Multica runs need loopback access to the local API/daemon
			// that powers `multica` CLI commands, so keep the Codex thread in
			// full-access mode instead of the default isolated workspace sandbox.
